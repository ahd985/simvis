...
export function summarizeCondition(condition, data, dataHeaders) {
    let summary = {};
    ...
    if (condition.level_data) {
        ...
        if (condition.levelDataSpan) {
            for (let j=0; j<condition.levelDataSpan; j++) {
                data = data.concat(data.map((e) => {return e[condition.levelDataIndex + j]}))
            }
        } else {
            data = this.props.data.map((e) => {return e[condition.levelDataIndex]})
        }

        const minVal = Math.min(...data);
        const maxVal = Math.max(...data);
        levelRange += minVal + " - " + maxVal
        const levelDataName = this.props.dataHeaderOptions[condition.levelDataIndex].text
        ...
        levelSummary = <div>
            ...
            <div className="minor-text">{"Bounds: " + valueMap.minHeight + " - " + valueMap.maxHeight}</div>
        </div>;
...
class LevelForm extends Component {
    ...
    render() {
        ...
        if (valueMap.levelDataIndex) {
            let data = [];
            if (dims > 1) {
                for (let j=0; j<valueMap.levelDataSpan; j++) {
                    data = data.concat(this.props.data.map((e) => {return e[valueMap.levelDataIndex + j]}))
                }
            } else {
                data = this.props.data.map((e) => {return e[valueMap.levelDataIndex]})
            }

            const minVal = Math.min(...data);
            const maxVal = Math.max(...data);
            levelRange += minVal + " - " + maxVal
            const levelDataName = this.props.dataHeaderOptions[valueMap.levelDataIndex].text

            const span = valueMap.levelDataSpan ? valueMap.levelDataSpan : 1
